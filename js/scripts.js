...
var html = '';
var i;
var stats = document.getElementById('table-content');
...
function buildTable(){
  ...
  for(i=0; i<players.length;i++){
    ...
    html += '<tr><td><strong>' + players[i].first + '</strong></td><td>' + players[i].last + '</td><td>' +
    players[i].position + '</td><td>' + players[i].year + '</td></tr>';
  }else{
    html += '<tr><td>' + players[i].first + '</td><td>' + players[i].last + '</td><td>' +
    players[i].position + '</td><td>' + players[i].year + '</td></tr>';
  };
  }
  stats.innerHTML = html; // everything is now stored in this function, you need to PUT it in there
}
